...
| `VITE_PLAUSIBLE_DOMAIN` | nie | prázdne (analytika vypnutá) | Master feature-flag pre analytiku. Doména registrovaná v Plausible (napr. `pawly.sk`). Kým nie je nastavená, `main.tsx` neinjektuje skript a `track()` je no-op. Cookieless, GDPR-friendly — **žiadny cookie banner netreba**. Skript sa načíta **len v PROD**. Konzumuje sa cez `utils/analytics.ts`. Meria pageviews automaticky + funnel eventy (`cta_start`, `cta_register`, `cta_profile`, `cta_analyze`, `demo_sample`, `food_query`) na landingu. |
| `VITE_PLAUSIBLE_SRC` | nie | `https://plausible.io/js/script.js` | Voliteľné — URL Plausible skriptu. Preplň len pri self-hoste alebo proxy cez vlastnú doménu (obchádza adblockery). Formát `script.js` (podporuje custom eventy cez `window.plausible`). |
