...
- **`articles`** je výnimka — **verejný globálny obsah** (články poradne) BEZ vlastníka, na rozdiel od per-user/per-pet tabuliek. Číta sa cez verejný read endpoint `GET /api/articles[/:slug]` (mountnutý pred `firebaseAuth`, bez tokenu). Zdroj pravdy pre seed/fallback je `client/src/content/poradna/articles.ts`; migrácia `0018_articles.sql` + seed `0019_articles_seed.sql`. RLS deny-by-default (service_role obchádza). Prerender zatiaľ číta z `articles.ts`; build-time napojenie na DB je plánovaný ďalší krok.
